...
describe("BigBanger", function () {
  ...
  it("should set correct state variables", async function () {
    this.banger = await this.BigBanger.deploy(this.sushi.address, this.dev.address, "1000", "0", "1000")
    await this.banger.deployed()

    await this.sushi.transferOwnership(this.banger.address)

    const sushi = await this.banger.relict()
  ...
  context("With ERC/LP token added to the field", function () {
    ...
    it("should allow emergency withdraw", async function () {
      ...
      this.banger = await this.BigBanger.deploy(this.sushi.address, this.dev.address, "100", "100", "1000")
      await this.banger.deployed()
    ...
    it("should not distribute SUSHIs if no one deposit", async function () {
      ...
      this.banger = await this.BigBanger.deploy(this.sushi.address, this.dev.address, "100", "200", "1000")
      await this.banger.deployed()
      await this.sushi.transferOwnership(this.banger.address)
      await this.banger.add("100", this.lp.address, true)
      await this.lp.connect(this.bob).approve(this.banger.address, "1000")
    ...
    it("should distribute SUSHIs properly for each staker", async function () {
      ...
      this.banger = await this.BigBanger.deploy(this.sushi.address, this.dev.address, "100", "300", "1000")
      await this.banger.deployed()
      await this.sushi.transferOwnership(this.banger.address)
      await this.banger.add("100", this.lp.address, true)
      ...
      expect(await this.sushi.balanceOf(this.banger.address)).to.equal("4334")
      expect(await this.sushi.balanceOf(this.dev.address)).to.equal("1000")
      ...
      expect(await this.sushi.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.sushi.balanceOf(this.banger.address)).to.equal("8144")
    ...
    it("should give proper SUSHIs allocation to each pool", async function () {
      ...
      this.banger = await this.BigBanger.deploy(this.sushi.address, this.dev.address, "100", "400", "1000")
      await this.sushi.transferOwnership(this.banger.address)
    ...
    it("should stop giving bonus SUSHIs after the bonus period ends", async function () {
      ...
      this.banger = await this.BigBanger.deploy(this.sushi.address, this.dev.address, "100", "500", "600")
      await this.sushi.transferOwnership(this.banger.address)
      await this.lp.connect(this.alice).approve(this.banger.address, "1000", { from: this.alice.address })
